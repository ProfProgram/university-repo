...
public class WordGuessMain {
	...
	protected void askUserToGuess() {
		...
		if (candidates.isEmpty()) {
			...
			if (candidates.contains(guess)) {
				...
				System.out.println("Incorrect, a correct word would have been ...");
	...
	protected boolean processLine(String pattern) {
		...
		if (cmd == '+') {
			Set<String> foundWords = wordList.searchWords(firstLetter, substring);
			candidates.addAll(foundWords);
		}
		else if (cmd == '-') {
			Set<String> foundWords = wordList.searchWords(firstLetter, substring);
			candidates.removeAll(foundWords);
		}
		else if (cmd == '*') {
			Set<String> foundWords = wordList.searchWords(firstLetter, substring);
			candidates.retainAll(foundWords);
		}
		else {
